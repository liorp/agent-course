...
beginnerConcepts:
  ...
  - question: "מה זה הזרקת זהות מחדש?"
    answer: "לאחר שדחיסת context (s06) מוחקת את היסטוריית השיחה, הסוכן עשוי לשכוח מי הוא. ה-harness מזריק בלוק <identity> בתחילת ה-context הדחוס: 'אתה Alice, מומחית backend, עובדת על משימה 3.' זה משחזר את תחושת הזהות של הסוכן."
...
ב-s09-s10, חברי הצוות עובדים רק כשנאמר להם במפורש. המוביל חייב לשגר כל אחד עם פרומפט ספציפי. 10 משימות לא תפוסות בלוח? המוביל מקצה כל אחת ידנית. לא מתרחב.

אוטונומיה אמיתית: חברי הצוות סורקים את לוח המשימות בעצמם, תופסים משימות לא תפוסות, עובדים עליהן, ואז מחפשים עוד.

פרט עדין אחד: לאחר דחיסת context (s06), הסוכן עשוי לשכוח מי הוא. הזרקת זהות מחדש פותרת זאת.
...
3. הזרקת זהות מחדש עוטפת את פונקציית ה-hard_compact מ-s06.
