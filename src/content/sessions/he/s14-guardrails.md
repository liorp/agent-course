...
challenges:
  - tier: "warmup"
    text: "רשמו 3 דרכים לעקוף את הרשימה השחורה מ[לולאת הסוכן](/he/s01-the-agent-loop) (`['rm -rf /', 'sudo']`). ואז הסבירו למה הרשאות מבוססות יכולות לא סובלות מהחורים האלה."
    hint: "חשבו על: משתני סביבה ($SHELL), קידוד, שרשור פקודות (&&), ו-aliases."
  - tier: "build"
    text: "ממשו את מערכת ה-guardrail המלאה: הוסיפו פונקציית `human_approve()` שמדפיסה את קריאת הכלי הממתינה ומחכה לקלט y/n. שלבו אותה בלולאת הסוכן מ[לולאת הסוכן](/he/s01-the-agent-loop)."
